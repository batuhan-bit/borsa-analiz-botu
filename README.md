...
verisi (Alpha Vantage) + bağımsız web duygusu (Perplexity, opsiyonel çapraz
doğrulama — iki kaynak ters düşerse sinyalde "⚠️ Kaynaklar çelişkili" uyarısı).
...
├── data/              # Alpaca · yfinance · Alpha Vantage · Perplexity istemcileri
...
| `PERPLEXITY_API_KEY` | Opsiyonel | Yoksa çapraz doğrulama atlanır, yalnızca AV kullanılır (ücretli — cap `max_symbols_per_run` ile sınırlı) |
